...
| **截图 OCR 取词** | `Ctrl+Shift+O` 框选扫描件/字幕，系统本地 OCR，不上传 |
...
在任意应用中选中英文文本，按 **Ctrl+C** 复制，查词窗口会自动弹出。**VS Code / IDEA / 浏览器 / PDF 均默认支持**。
...
如果你想了解产品设计意图、功能取舍的理由，或参与后续开发，可以从这些文档入手：

| 文档 | 内容 | 适合谁看 |
|------|------|----------|
| [v1.0.0 实现版 PRD](docs/PRD-v1.0.0-as-built.md) | 首版实际形态：功能范围、用户流程、技术选型 | 想快速理解产品全貌 |
| [后续功能规划](docs/ROADMAP-v1.1-v1.3.md) | 竞品分析、差异化定位、v1.1–v1.3 路线，以及明确不做的功能与理由 | 想了解产品方向 |
| [v1.1 开发规格](docs/PRD-v1.1-spec.md) | 数据目录修复、缓存、流式、剪贴板治理、DPAPI | 想参与开发 |
| [v1.2 开发规格](docs/PRD-v1.2-spec.md) | schema 迁移、轻量复习、阅读会话与自动更新 | 想参与开发 |
...
- 敏感内容自动过滤：复制密码、密钥、文件路径、邮箱、IP 等不会触发查词，也不会写入缓存或发送给模型
- 应用黑名单默认只含密码管理器；**IDE / 终端默认可查**（设置里可关闭「在 IDE / 终端中查词」）
- 内容过滤采用强语境判定：`import numpy as np` 会被拦，`import the data from Excel`、`let me know` 会正常查词
...
- 本地统计（`local_events`）按日聚合，默认保留 90 天，可在设置中一键清除。
