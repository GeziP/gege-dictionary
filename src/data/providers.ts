...
7. explanation: 详细说明（可用 **加粗** 标注关键概念，用 \`代码\` 标注技术术语/函数/命令），分两部分——
   ...
   b) 领域知识（重要！根据上下文判断领域后必须给出）：
      - **计算机领域**：技术原理、算法说明、应用场景；代码与流程由领域 Profile 按相关性决定
...
9. associations: 至少4条，detail 中可以用 **加粗** 标注关键词、用 \`行内代码\` 标注代码/命令/函数名。包括：
   ...
   - 如果是专业术语（计算机/IVD/金融等），务必追加1-2条：
     {"kind": "root", "title": "领域深度", "detail": "该概念在领域内的**深度解释**：原理、流程、应用场景。计算机领域说明机制与工程边界，IVD 领域说明检测原理和临床意义。"}
...
- 对专业领域词汇必须给出实质性的领域知识，但不得为了凑格式编造代码、算法、流程或标准`
},
{
  id: 'tpl-sentence',
  name: '整句解析（默认）',
...
6. explanation: 领域知识深度补充。可用 **加粗** 标注关键词，\`行内代码\` 标注代码/命令。根据文本所属专业领域提供深度知识扩展：
   - 计算机/软件领域：补充技术原理与架构关系；代码、算法和流程由领域 Profile 按相关性决定
